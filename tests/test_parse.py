...
def test_parse_binary_precedence() -> None:
    assert parse_expr('a + b * c') == BinaryOp("+", Identifier("a"), BinaryOp("*", Identifier("b"), Identifier("c")))
    assert parse_expr('a * b + c') == BinaryOp("+", BinaryOp("*", Identifier("a"), Identifier("b")), Identifier("c"))
    assert parse_expr('a < b + c') == BinaryOp("<", Identifier("a"), BinaryOp("+", Identifier("b"), Identifier("c")))
    assert parse_expr('a + b < c') == BinaryOp("<", BinaryOp("+", Identifier("a"), Identifier("b")), Identifier("c"))
    assert parse_expr('a == b >= c') == BinaryOp("==", Identifier("a"), BinaryOp(">=", Identifier("b"), Identifier("c")))
    assert parse_expr('a >= b == c') == BinaryOp("==", BinaryOp(">=", Identifier("a"), Identifier("b")), Identifier("c"))
...
def test_parse_conditional() -> None:
    assert parse_expr('a ? b : c') == Conditional(Identifier("a"), Identifier("b"), Identifier("c"))
...
def test_parse_array() -> None:
    ...
    assert parse_expr('[1, 2, 3, 4]') == Array([Literal(1), Literal(2), Literal(3), Literal(4)])
...
def test_parse_object_complex() -> None:
    assert parse_expr('{ (foo) = bar }') == Object({Parenthesis(Identifier("foo")): Identifier("bar")})
    ...
    assert parse_expr('{ "for" = 1, baz = 2}') == Object({Literal("for"): Literal(1), Identifier("baz"): Literal(2)})
    assert parse_expr('{ baz = 2, for = 1}') == Object({Identifier("baz"): Literal(2), Identifier("for"): Literal(1)})
...
def test_parse_function_call() -> None:
    ...
    assert parse_expr('foo(1, 2, 3)') == FunctionCall(Identifier("foo"), [Literal(1), Literal(2), Literal(3)])
    assert parse_expr('foo(1, 2, 3...)') == FunctionCall(Identifier("foo"), [Literal(1), Literal(2), Literal(3)],
                                                         var_args=True)


def test_parse_get_attr() -> None:
    assert parse_expr('foo.bar') == GetAttr(Identifier("foo"), GetAttrKey(Identifier("bar")))


def test_parse_get_index() -> None:
    assert parse_expr('foo[0]') == GetIndex(Identifier("foo"), GetIndexKey(Literal(0)))
    assert parse_expr('foo[bar]') == GetIndex(Identifier("foo"), GetIndexKey(Identifier("bar")))


def test_parse_get_attr_splat() -> None:
    assert parse_expr('foo.*') == AttrSplat(Identifier("foo"), [])
    assert parse_expr('foo.*.bar') == AttrSplat(Identifier("foo"), [GetAttrKey(Identifier("bar"))])


def test_parse_index_splat() -> None:
    assert parse_expr('foo[*]') == IndexSplat(Identifier("foo"), [])
    assert parse_expr('foo[*].bar') == IndexSplat(Identifier("foo"), [GetAttrKey(Identifier("bar"))])
    assert parse_expr('foo[*][3]') == IndexSplat(Identifier("foo"), [GetIndexKey(Literal(3))])
    assert parse_expr('foo[*].bar[3]') == IndexSplat(Identifier("foo"),
                                                     [GetAttrKey(Identifier("bar")), GetIndexKey(Literal(3))])
...
def test_parse_for_tuple_expr() -> None:
    ...
    assert parse_expr("[for a in b: a if a]") == ForTupleExpression(
        key_ident=None, value_ident=Identifier("a"), collection=Identifier("b"), value=Identifier("a"),
        condition=Identifier("a"))
    assert parse_expr("[for a, b in c: a if a]") == ForTupleExpression(
        key_ident=Identifier("a"), value_ident=Identifier("b"), collection=Identifier("c"), value=Identifier("a"),
        condition=Identifier("a"))
...
def test_parse_block() -> None:
    ...
    """) == Module([
        Block("resource", [Identifier("a"), Literal("b")], [
            Attribute("a", Identifier("b"))
        ])
    ])
