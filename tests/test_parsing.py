...
def test_can_parse_read_from_attributes() -> None:
    assert parse_module("value = foo") == Module(
        [Attribute("value", Identifier("foo"))]
    )
    assert parse_module("value = foo.bar") == Module(
        [Attribute("value", GetAttr(Identifier("foo"), GetAttrKey(Identifier("bar"))))]
    )
    assert parse_module("value = foo.bar.baz") == Module(
        [
            Attribute(
                "value",
                GetAttr(
...
def test_can_parse_unary_operator() -> None:
    assert parse_module("value = !addend") == Module(
